...
GLWidget::GLWidget(QWidget *parent)
    : QGLWidget(parent), initialized_(false), num_instances(1), width_(0.0), height_(0.0), dist_offset(1.0), myLod(0),
...
// /////////////// Box BEGIN
      ...
      glBindVertexArray(skyboxVAO);
      glActiveTexture(GL_TEXTURE0);
      glDrawElements(GL_TRIANGLES, 30+3, GL_UNSIGNED_BYTE, (void*) 0);
      glBindVertexArray(0);
      glDepthFunc(GL_LESS);
...
// ////////////////////// MODEL PAINTING END
      if (myFPS == 0.0f)
          myFPS = 1.0f;
      ps_.updateParticleSystem( 0.1f, upd_method ); // ::EulerOrig | ::EulerSemi | ::Verlet

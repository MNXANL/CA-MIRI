...
Particle::Particle() :
m_currentPosition(0,0,0), m_previousPosition(0, 0, 0), m_velocity(0, 0, 0), m_force(0, 0, 0), m_bouncing(1), m_life(0), /*m_lifetime(50),*/ m_fixed(false), m_mass(1.0)
{
    m_lifetime = fRando(20, 80);
}

Particle::Particle(const float& x, const float& y, const float& z) :
m_previousPosition(0, 0, 0), m_velocity(0, 0, 0), m_force(0, 0, 0), m_bouncing(1), /*m_lifetime(50),*/ m_fixed(false), m_mass(1.0)
{
...
void Particle::updateParticle(const float& dt, UpdateMethod method)
{
	if (!m_fixed)
	{
		switch (method)
		{
            ...
            {
                // to be implemented (SPOILER: Implemented)
                float K = 0.99f;
                m_currentPosition = m_previousPosition + K*( m_currentPosition - m_previousPosition ) + ( m_force*(dt*dt) / m_mass );
...
{
    float dist = sqrt( pow((m_currentPosition.x - sph.center.x), 2) + pow((m_currentPosition.y - sph.center.y), 2) + pow((m_currentPosition.z - sph.center.z), 2) );
    return dist <= (sph.radius * sph.radius);
}
